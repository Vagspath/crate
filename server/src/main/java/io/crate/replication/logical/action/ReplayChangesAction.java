...
public class ReplayChangesAction extends ActionType<ReplicationResponse> {
    ...
    public static class TransportAction
        ...
        private void performOnPrimary(IndexShard primary, Request request, List<Engine.Result> accumulator, int offset, Consumer<List<Engine.Result>> result, Consumer<Exception> failure) {
            for (int itemsToProcess = offset; itemsToProcess < request.changes().size(); itemsToProcess++) {
                final var op = request.changes().get(itemsToProcess);
                ...
                if (engineResult.getResultType() == Engine.Result.Type.MAPPING_UPDATE_REQUIRED ||
                    ...
                    retryWhenMappingsAreUpdated(primary, request, accumulator, itemsToProcess, result, failure);
                    return;
                }
        ...
        private void retryWhenMappingsAreUpdated(IndexShard primary, Request request,List<Engine.Result> accumulator, int position, Consumer<List<Engine.Result>> result, Consumer<Exception> failure) {
            ...
            clusterStateObserver.waitForNextChange(
                new ClusterStateObserver.Listener() {
                    ...
                    public void onNewClusterState(ClusterState state) {
                        performOnPrimary(primary, request, accumulator, position, result, failure);
                    }
